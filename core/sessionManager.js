...
export async function addCharacterToSession(sessionName, character) {
  ...
  let sessionCharacterCheck = await db.checkifCharacterInSession(
    sessionName,
    character.id
  );
...
export async function addItemBidtoRaidLedger(
  charName,
  itemId,
  itemName,
  dkpAmount,
  ...
  let update = await db.insertSessionLedger(
    sessionName,
    character.raid_team,
    character.id,
    itemName,
...
export async function updateSessionLedger(sessionId, update) {
  ...
  if (update.itemId !== 0) {
    ...
    console.log(`Updating ${character.name} dkp to ${newdkpAmount}.`)
